...
contains
  ...
  subroutine flow_calc(phis,lxyz,lxyz_inv, Lsize, np)    
   ...
   !    do i=1, path_length_v(n)
          ...
          ! ip, m
   !       hydro(Lmn(i, n))%ip = path_ip(i)
   !       hydro(Lmn(i, n))%m = MN_nodes(1)
   !       hydro(Lmn(i, n))%n = MN_nodes(2)
          !write(*,'(I10,I10,I10)') hydro(path_ip(i))%ip , hydro(path_ip(i))%m, hydro(path_ip(i))%n
   ...
   ! end do
    ...
    do m=1, np_nodes
       do n=1, np_nodes
         ...
         ! length_vertex_connections(nodes_vertex_id(m), nodes_vertex_id(n)) = &
         !      length_vertex_connections(nodes_vertex_id(m), nodes_vertex_id(n)) +  real(nodes_matrix(m,n))
    ...
    !do i=1, n_vertex
    !   write(*,*) permittivity(i,:)
    !end do
    call DGESV(np_nodes, 1, permittivity, np_nodes, IPIV, B, np_nodes, ierr)
    !write(*,*) "n_vertex, np_nodes", n_vertex, np_nodes
    write(*,*) "B", B(1:np_nodes)
    write(*,*) "ierr status dgesv", ierr
   ...
   ! OPEN (UNIT=1099291,FILE='vertex')
   ! do i=1, n_vertex
   !    write(1099291,'(69F10.2)', advance='no') permittivity(i,:)
   ! end do
    !close(1099291)
   ! write(*,*) "done!"
    ...
    do ip=1, np_skel
       ...
       write(109929,'(I5,I5,I5,F10.2,F10.5)')  i,j,k, phis(hydro(ip)%ip), hydro(ip)%flow       
