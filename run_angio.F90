    ...
    subroutine etc_move(cell, tipc, gg, lxyz, lxyz_inv, Lsize, vegf_c, vegf_grad_min, vegf_grad_max, tip_s, &
      ...
      do ip2 = 1, n_tipcell
         ...
         if(grid_cell_domain(ip)> 0) then
            ...
            relative_pos(1:3) = lxyz(ip,1:3) - vegf_xyz(grid_cell_domain(ip),1:3) 
            relative_image(1:ndim) = 2.d0*Lsize(1:ndim) -unitary(1:ndim)- abs(relative_pos(1:ndim))
          
  !          distance(1:3) = relative_pos(1:3)
...
!            distance(1:3) = distance(1:3)/(sqrt(distance(1)**2+ distance(2)**2  + distance(3)**2))

!            GG_projection = grad(1)*distance(1) + grad(2)*distance(2) + grad(3)*distance(3)
