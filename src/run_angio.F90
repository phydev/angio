...
contains
  ...
  subroutine run_angio(sim_id)
    ...
    ! file_id = '92000'
    !                     field, ...,    ..., ... , directory, ...
    ! call init_from_file(cell, lxyz_inv, np, 446745, '001',  file_id)
    ...
    do while(nstep<=tstep)
      ...
      call dderivatives_lapl(cell(1:np)%mu, cell(1:np)%lapl_mu, np, dr, lxyz, lxyz_inv)


      ! Calculating laplacian of T
      call dderivatives_lapl(cell(1:np)%T, cell(1:np)%lapl_T, np, dr, lxyz, lxyz_inv)
      ...
      if(counter.ge.output_period) then
        write(*,*) nstep
        counter = 0
        ...
        do ip=1, np
          ...
          if(cell(ip)%phi>0) then
            write(500,'(I10,I10,I10,F10.2,F10.4)') lxyz(ip,1:3), cell(ip)%phi, flow_full(ip)
          end if
          if(lxyz(ip,3).eq.0) then
            write(700,'(I10,I10,F10.3)') lxyz(ip,1:2), cell(ip)%T
          end if
        end do
        close(500)
        close(700)
